...
8. Company→CIK resolution must handle **delisted targets**. Resolve names via EDGAR full-text search (`efts.sec.gov`, covers filers back to 2001) as the primary matcher, fall back to `company_tickers.json` fuzzy matching, and consult a hand-verified override table first (`--cik-overrides cik_manual_overrides.csv`). Raise `--min-name-score` (≈90) to suppress wrong-company matches on generic bank/financial names; audit the universe with `audit_cik_resolution.py`.
